...
def on_input(sender,data):
	input_text_value = dpg.get_value("search")
	song_data = main.recommend_songs(input_text_value)
	song_data_str = str(song_data)
	dpg.add_text(song_data_str, parent="Recommend_bar")
...
with dpg.window(tag="main", label="window title"):
	...
	with dpg.group(horizontal=True):
		...
		with dpg.child_window(autosize_x=True, border=False):
			...
			with dpg.child_window(autosize_x=True, height=265, delay_search=True):
			...
			with dpg.group(horizontal=True):
				with dpg.child_window(height=240, tag="Recommend_bar"):
					dpg.add_text("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tSuggestions", color=(255, 255, 0))
					dpg.add_input_text(id="search", hint="search Song", width=-1)
					dpg.add_button(label="Recommend", callback=on_input)
